...
router.get('/:id', (req, res) => {
  ...
  data.Result.findOne({
    ...
    if (err instanceof Sequelize.ValidationError || err instanceof Sequelize.ForeignKeyConstraintError) {
      message = err.message;
...
router.get('/:id/summary', async (req, res) => {
  ...
  try {
    ...
    var resultData = await data.Result.findOne({
      ...
      attributes: {
        include: [[Sequelize.fn('COUNT', Sequelize.col('resultId')), 'totalQuestions']]
      }
    });
...
router.get('/test/:testId/summary', async (req, res) => {
  ...
  try {
    ...
    const result_answer_counts_dh2q = await data.Result.findAll({
      where: {
  ...
  try {
    ...
    for (i = 0; i < Object.keys(req.body.answers).length; i++) {
      ...
      await data.Answer.create({
        ...
        correctlyAnswered: answerData.studentAnswer == eval(answerData.operand1 + answerData.operation + answerData.operand2),
    ...
    var uri = req.protocol + '://' + req.get('host') + req.baseUrl + req.path + newResult.id;
    result['resource'] = {
      'id': newResult.id,
      'uri': uri
    };
